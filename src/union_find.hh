...
class UnionFind {
    ...
    int find(int id); // Returns the identifier of the set that contains the parameter id
    void doUnion(int id_1, int id_2); // Merges the two sets given by the identifiers
...
UnionFind::~UnionFind(){
    for(pair<int,UF_element*> p : idToElement) delete p.second;
}
...
int UnionFind::getSize(int id){
    return idToElement.at(find(id))->size;
}
